...
- ✅ 보안 감사 완료 (44건+ 수정)
- ✅ **v49 음질 최적화 (최신)**:
  - v49 핵심: f0_autotune ON, hop 128, 한/영 EQ 분리, 3kHz boost 제거
  - 고음/가성 안정화: autotune + hop 128 + filter 3 + protect 0.40
  - 치찰음 감소: 3kHz presence 제거 + 좁은 8kHz 디에서 + 한/영 분리
...
| F0 | RMVPE (+ **f0_autotune=True, strength=0.4**) | v53: 0.6→0.4 (비브라토 보존) |
| **index_rate** | **0.45** (한국어 0.50 / 영어 0.35) | v51: 0.55→0.45 |
| **rms_mix_rate** | **0.20** | v53: 0.1→0.20 (원곡 음량 패턴 20% 반영) |
| **protect** | **0.40** | v49: 0.33→0.40 |
| **filter_radius** | **3** | v53: 2→3 (가성 안정화, median 28ms) |
...
| 후처리 (한국어) | agate + HPF 80Hz + 1.2kHz -0.5 + 5kHz -1.5 + 8kHz -0.8 + 디에서 2단(6.5k+9k) + loudnorm | v53 |
| 후처리 (영어) | agate + HPF 80Hz + 300Hz -0.3 + 600Hz -0.3 + 5kHz -0.8 + 1.2kHz -0.5 + 5kHz -1.5 + 8kHz -0.8 + 디에서 2단 + loudnorm | v53 |
| 믹스 후처리 | 리미터 0.89 + **LUFS -14 정규화** (2-pass) | v53: 믹스 LUFS 정규화 추가 |
...
- Seed-VC로 고음역 학습 데이터 증강 (C5-C6 커버리지)
- **Spin V2 embedder**: ContentVec 대비 발음+숨소리 개선 (재학습 필수, A/B 테스트)
- **Mel-RoFormer**: BS-Roformer 대비 +0.5dB SDR (보컬 분리 품질 향상)
